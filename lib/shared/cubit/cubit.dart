...
/*   String themeMode = 'system';
  ...
  ThemeMode changeAppMode({String? fromShared}) {
    if (fromShared != '') {
      if (fromShared == 'dark') {
        themeMode = fromShared ?? 'dark';
        theme = ThemeMode.dark;
        print('Dark Mode');
        emit(DarkModeState());
        CacheHelper.saveData(key: 'themeMode', value: themeMode)
            .then((value) => emit(DarkModeState()));
      } else if (fromShared == 'light') {
        themeMode = fromShared ?? 'light';
        theme = ThemeMode.light;
        emit(LightModeState());
        CacheHelper.saveData(key: 'themeMode', value: themeMode)
            .then((value) => emit(LightModeState()));
      } else if (fromShared == 'system') {
        themeMode = fromShared ?? 'system';
        theme = ThemeMode.system;
        emit(SystemModeState());
        CacheHelper.saveData(key: 'themeMode', value: themeMode)
            .then((value) => emit(SystemModeState()));
      }
    } else {
